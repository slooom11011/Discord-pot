...
async def on_member_ban(guild, user):
    if log:=discord.utils.get(guild.channels, name=CFG["log"]):
        e=discord.Embed(title="🔨 تم تبنيد عضو", color=0x992d22, timestamp=datetime.now(timezone.utc))
        e.add_field(name="العضو", value=f"{user.mention}\n`{user.name}`", inline=True)
...
async def سيرفر(ctx):
    ...
    e.add_field(name="🤖 البوتات", value=f"`{bots}`", inline=True)
    e.add_field(name="📈 الإجمالي", value=f"`{g.member_count}`", inline=True)
    e.add_field(name="👑 أعلى لفل", value=top_text, inline=True)
    e.add_field(name="📅 تاريخ الإنشاء", value=f"<t:{int(g.created_at.timestamp())}:R>", inline=True)
    e.add_field(name="🔧 البوت", value=f"`{round(bot.latency*1000)}ms`", inline=True)
...
async def طرد(ctx, m:discord.Member, *, reason="مافي سبب"):
    ...
    if log:=discord.utils.get(ctx.guild.channels, name=CFG["log"]):
        e=discord.Embed(title="👢 تم طرد عضو", color=0xe74c3c, timestamp=datetime.now(timezone.utc))
        e.add_field(name="العضو", value=f"{m.mention}\n`{m.name}`", inline=True)
        e.add_field(name="المشرف", value=ctx.author.mention, inline=True)
...
async def باند(ctx, m:discord.Member, *, reason="مافي سبب"):
    await m.ban(reason=reason)
    await ctx.send(embed=discord.Embed(description=f"🔨 تم تبنيد {m.mention}\n**السبب:** {reason}", color=0x992d22))
