...
async def update_role(m,lvl):
    [await m.remove_roles(r) for r in m.roles if r.name in [d[0] for d in CFG["lvl_roles"].values()]]
    for lv,(n,c) in sorted(CFG["lvl_roles"].items(),reverse=True):
        if lvl>=lv: await m.add_roles(await get_role(m.guild,n,c)); return
    ...
    if uid not in xp_cd or (datetime.now(timezone.utc)-xp_cd[uid]).seconds>=60:
        ...
        if new_lvl>lvl:
            if ch:=discord.utils.get(msg.guild.channels,name=CFG["lvl_up"]):
                ...
                e.set_footer(text=msg.guild.name,icon_url=msg.guild.icon.url if msg.guild.icon else None); await ch.send(embed=e)
            if r:=await update_role(msg.author,new_lvl): await msg.channel.send(f"🎊 مبروك {msg.author.mention} حصلت على رول {r.mention}")
